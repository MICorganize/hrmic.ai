  ...
  return (
    <div className="h-[calc(100vh-4rem)] overflow-hidden bg-[#e7eff8]">
      <section className="flex min-h-40 items-start justify-between gap-5 bg-[#61a8ff] px-6 pb-6 pt-12 text-white">
        <div>
          <div className="flex items-center gap-1 text-sm text-white/90"><span>ประมวลผลเงินเดือน</span><ChevronRight className="size-4" /><span>คำนวณงวดพิเศษ</span></div>
      ...
      {addOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/25 px-4"><div role="dialog" aria-modal="true" className="w-full max-w-lg rounded-lg bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-slate-200 px-6 py-4"><h2 className="text-xl font-semibold text-[#303943]">เพิ่มงวดพิเศษ</h2><button type="button" onClick={() => setAddOpen(false)} aria-label="ปิด"><X className="size-5" /></button></div><div className="space-y-4 px-6 py-5"><label className="block text-sm font-medium text-slate-700">ชื่องวดพิเศษ<input className="mt-1 h-10 w-full rounded border border-slate-300 px-3 font-normal outline-none" /></label><label className="block text-sm font-medium text-slate-700">วันที่จ่าย<span className="mt-1 block"><ThaiDatePicker value={paymentDate} onChange={setPaymentDate} /></span></label></div><div className="flex justify-end gap-2 border-t border-slate-200 px-6 py-4"><button type="button" onClick={() => setAddOpen(false)} className="h-9 rounded border border-slate-300 px-4 text-sm">ยกเลิก</button><button type="button" onClick={() => setAddOpen(false)} className="h-9 rounded bg-[#168ee8] px-4 text-sm text-white">บันทึก</button></div></div></div>}
    </div>
  );
}
