  ...
  return <div className="min-h-[calc(100vh-70px)] bg-[#f3f6fb] font-sans">
    <div className="mx-auto max-w-[1600px] p-3 sm:p-4">
      <section className="mb-3 overflow-hidden rounded-xl border border-[#e5eaf2] bg-white shadow-[0_3px_12px_rgba(29,52,93,.07)]">
        <div className="flex flex-col gap-3 px-4 py-3 text-sm leading-5 lg:flex-row lg:items-center">
          ...
          {!closed && <div className="flex h-8 flex-1 items-center rounded-lg border border-[#f3dda0] bg-[#fff9df] px-3 py-0 text-[14px] font-[400] leading-5 tracking-[-0.4px] text-[#755c18] lg:self-start"><span>คุณกำลังคำนวณเงินเดือนของเดือน &quot;{label}&quot; เมื่อคำนวณเสร็จแล้ว กรุณาปิดงวดบัญชี</span><button className="ml-[2px] font-medium text-[#1474ee] hover:underline" onClick={() => setHandoff({ tab: "ปิดงวดบัญชี", month: INITIAL_PAYROLL_MONTH_KEY })}>คลิกที่นี่</button></div>}

          <div className="w-full shrink-0 sm:w-80">
            <ThaiMonthPicker aria-label="เลือกเดือนคำนวณเงินเดือน" value={INITIAL_PAYROLL_MONTH_KEY} onChange={(month) => setHandoff({ tab: "Dashboard", month })} className="h-8 w-full rounded-lg border border-[#dfe4e8] bg-white px-3 text-sm font-normal text-[#34425c] outline-none ring-[#5eaafa] focus:ring-2" />
            <div className="mt-1 flex h-6 items-center justify-between">
              <span className="relative left-[8px] flex min-w-0 flex-1 justify-center whitespace-nowrap text-[13px] leading-5 text-[#6f7b90]">{period ? formatPayrollPeriod(period.start, period.end) : label}</span>
