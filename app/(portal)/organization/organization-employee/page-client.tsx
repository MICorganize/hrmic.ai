...
function OrganizationTreeDropdown({ companies, value, loading, onChange }: { companies: OrganizationRecord[]; value: string; loading: boolean; onChange: (value: string) => void }) {
  ...
  const renderNode = (node: OrganizationRecord, depth = 0): React.ReactNode => {
    ...
    return <li key={`${node.kind}-${node.id}`} role="treeitem" aria-level={depth + 1} aria-expanded={hasChildren ? isExpanded : undefined}><div className="flex min-h-6 items-center rounded-none pr-1 hover:bg-[#f5f5f5]"><span aria-hidden className="h-6 shrink-0" style={{ width: `${depth * 18}px` }} />{hasChildren ? <button type="button" onClick={() => setExpanded((current) => { const next = new Set(current); next.has(node.id) ? next.delete(node.id) : next.add(node.id); return next; })} className="flex size-6 shrink-0 items-center justify-center text-[#666] hover:text-[#1677ff]" aria-label={isExpanded ? `ยุบ ${node.name}` : `ขยาย ${node.name}`}>{isExpanded ? <ChevronDown className="size-3.5" /> : <ChevronRight className="size-3.5" />}</button> : <span className="size-6 shrink-0" />}<button type="button" disabled={hasChildren} onClick={() => { onChange(node.id); setOpen(false); }} className={cn("min-w-0 flex-1 rounded-none px-1 py-0 text-left text-sm font-light leading-6", hasChildren ? "cursor-not-allowed text-black/25" : "text-black/85 hover:bg-[#e6f7ff]", value === node.id && "bg-[#e6f7ff] font-bold text-[#1677ff]")} title={node.name}>{node.name}</button></div>{hasChildren && isExpanded && <ul role="group" className="m-0 list-none p-0">{(node.children ?? []).map((child) => renderNode(child, depth + 1))}</ul>}</li>;
  };
  return <Popover open={open} onOpenChange={setOpen}><PopoverTrigger asChild><button type="button" disabled={loading || companies.length === 0} className={cn("flex h-8 w-full items-center justify-between rounded-[2px] border border-[#d9d9d9] bg-white px-[11px] text-left text-sm leading-[22px] text-black/85 shadow-none transition-colors focus-visible:outline-none focus-visible:border-[#40a9ff] focus-visible:shadow-[0_0_0_2px_rgba(24,144,255,0.2)] disabled:cursor-not-allowed disabled:bg-[#f5f5f5] disabled:text-black/25", open && "border-[#5eaafa] ring-2 ring-[#5eaafa]/20")}><span className={cn("truncate", !selectedNode && "text-black/25")}>{loading ? "กำลังโหลดโครงสร้างองค์กร..." : selectedNode?.name ?? "ข้อมูลองค์กร"}</span><ChevronDown className={cn("ml-2 size-4 shrink-0 text-black/25 transition-transform", open && "rotate-180")} /></button></PopoverTrigger><PopoverContent align="start" sideOffset={0} className="max-h-[280px] w-[var(--radix-popover-trigger-width)] overflow-auto rounded-[2px] bg-white p-1 shadow-[0_3px_6px_-4px_rgba(0,0,0,0.12),0_6px_16px_0_rgba(0,0,0,0.08),0_9px_28px_8px_rgba(0,0,0,0.05)] ring-0"><ul role="tree" aria-label="ข้อมูลองค์กร" className="m-0 list-none p-0">{companies.map((company) => renderNode(company))}</ul></PopoverContent></Popover>;
}
                ...
                <div className="m-2 min-w-0 flex-1">
                  ...
                  <div className="my-2 flex w-full gap-2">
                    <div className="mr-2 flex flex-1 items-center justify-center">
                      <DropdownSelect value={templateOrganizationId} onChange={(event) => setTemplateOrganizationId(event.target.value)} aria-label="โครงสร้างองค์กร" className="h-8 w-full rounded-[4px] border border-[#d9d9d9] bg-white px-3 text-sm text-muted-foreground outline-none focus:border-[#2299ff] focus:ring-1 focus:ring-[#2299ff]">
        ...
        <section>
          ...
          <div className="mx-8 mt-2">
            <div className="mb-4">
              ...
              <DropdownSelect value={position} onChange={(event) => setPosition(event.target.value)} className="h-8 w-full rounded-[4px] border border-[#d9d9d9] bg-white px-3 text-sm leading-[22px] text-foreground outline-none focus:border-[#2299ff] focus:ring-1 focus:ring-[#2299ff]">
        ...
        <div className="m-6">
          <div className="mb-2 flex flex-col gap-2 sm:flex-row sm:items-end">
            <label className="flex min-w-0 flex-1 flex-col text-sm leading-[22px] text-black/87">โครงสร้างองค์กร
              <DropdownSelect value={organizationId} onChange={(event) => setOrganizationId(event.target.value)} className={filterControlClass}>
            ...
            <Table className="min-w-full table-fixed font-[Kanit,sans-serif] text-sm leading-[22.001px]">
              ...
              <TableBody>
                {loading ? <TableRow><TableCell colSpan={BASIC_COLUMNS.length} className="h-24 text-center text-black/45">กำลังโหลดข้อมูล...</TableCell></TableRow> : visibleRows.length === 0 ? <TableRow><TableCell colSpan={BASIC_COLUMNS.length} className="h-24 text-center text-black/45">ไม่มีข้อมูล</TableCell></TableRow> : visibleRows.map((row, index) => <TableRow key={row.id} className={cn("!h-[50.4px] border-b-0 hover:bg-transparent", index % 2 === 0 ? "[&>td]:bg-[#f2fafe]" : "[&>td]:bg-white")}>
                  ...
                  <TableCell className={cellClass}><DropdownSelect value={row.title} onChange={(event) => updateRow(row.id, "title", event.target.value)} className={tableSelectClass}><option value="">-</option>{["นาย", "นาง", "นางสาว", "ดร."].map((title) => <option key={title}>{title}</option>)}</DropdownSelect></TableCell>
                  ...
                  <TableCell className={cellClass}><DropdownSelect value={row.gender} onChange={(event) => updateRow(row.id, "gender", event.target.value)} className={tableSelectClass}>{["ชาย", "หญิง", "ไม่ระบุ"].map((value) => <option key={value}>{value}</option>)}</DropdownSelect></TableCell>
                  <TableCell className={cellClass}><DropdownSelect value={row.maritalStatus} onChange={(event) => updateRow(row.id, "maritalStatus", event.target.value)} className={tableSelectClass}><option value="">-</option>{["โสด", "สมรส", "หย่าร้าง", "หม้าย"].map((value) => <option key={value}>{value}</option>)}</DropdownSelect></TableCell>
      ...
      <CardContent className="card-input-body px-2 py-4">
        <div className="m-6">
            ...
            <Table className="min-w-[3270px] table-fixed font-[Kanit,sans-serif] text-sm leading-[22.001px]">
              ...
              <TableBody>
                {loading ? <TableRow><TableCell colSpan={SALARY_COLUMNS.length} className="h-24 text-center text-black/45">กำลังโหลดข้อมูล...</TableCell></TableRow> : visibleRows.length === 0 ? <TableRow><TableCell colSpan={SALARY_COLUMNS.length} className="h-24 text-center text-black/45">ไม่มีข้อมูล</TableCell></TableRow> : visibleRows.map((row, index) => <TableRow key={row.id} className={cn("!h-[50.4px] border-b-0 hover:bg-transparent", index % 2 === 0 ? "[&>td]:bg-[#f2fafe]" : "[&>td]:bg-white")}>
                  ...
                  <TableCell className={cellClass}><DropdownSelect value={row.employeeType} onChange={(event) => updateRow(row.id, "employeeType", event.target.value)} className={tableSelectClass} aria-label={`ประเภทพนักงานของ ${row.name}`}><option value={row.employeeType}>{row.employeeType ? `ET0001: ${row.employeeType}` : ""}</option></DropdownSelect></TableCell>
                  ...
                  <TableCell className={cellClass}><DropdownSelect value={row.advanceType} onChange={(event) => updateRow(row.id, "advanceType", event.target.value)} className={tableSelectClass}><option value="">-</option>{["กำหนดวงเงินเบิกล่วงหน้า", "ไม่กำหนดวงเงินเบิกล่วงหน้า"].map((value) => <option key={value}>{value}</option>)}</DropdownSelect></TableCell>
          ...
          <TableCell className={cellClass}><ThaiDatePicker required value={row.confirmationDate} onChange={(value) => updateRow(row.id, "confirmationDate", value)} className={tableInputClass} /></TableCell>
                  ...
                  <TableCell className={cellClass}><DropdownSelect value={row.socialSecurityCalc} onChange={(event) => updateRow(row.id, "socialSecurityCalc", event.target.value)} className={tableSelectClass}><option value="">-</option>{["ไม่คิดประกันสังคม", "คิดประกันสังคม", "กำหนดค่าคงที่"].map((value) => <option key={value}>{value}</option>)}</DropdownSelect></TableCell>
                  ...
                  <TableCell className={cellClass}><DropdownSelect value={row.taxCalc} onChange={(event) => updateRow(row.id, "taxCalc", event.target.value)} className={tableSelectClass}><option value="">-</option>{["คิดภาษี ภงด.1 ใหม่ทุกเดือน", "คิดตามฐานเงินเดือนจริงที่ได้รับ", "ไม่คิดภาษี"].map((value) => <option key={value}>{value}</option>)}</DropdownSelect></TableCell>
        ...
        <div>
          <div className="mb-2 flex flex-col gap-2 lg:flex-row lg:items-end">
            ...
            <label className="flex min-w-0 flex-1 flex-col text-sm leading-[22px] text-black/87">กลุ่มผู้ใช้งาน
              <DropdownSelect value={userGroup} onChange={(event) => setUserGroup(event.target.value)} className={filterControlClass}>
      ...
      <CardContent className="card-input-body px-2 py-4">
        <div className="flex flex-col gap-2">
          ...
          <div className="mb-2 grid gap-2 lg:grid-cols-[1fr_1fr_1fr_auto] lg:items-end">
            <label className="flex flex-col text-sm leading-[22px] text-[rgba(0,0,0,0.87)]">โครงสร้างองค์กร<DropdownSelect value={organizationId} onChange={(event) => setOrganizationId(event.target.value)} className={controlClass}><option value="">โครงสร้างองค์กร</option>{organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DropdownSelect></label>
            <label className="flex flex-col text-sm leading-[22px] text-[rgba(0,0,0,0.87)]">ตำแหน่ง<DropdownSelect value={position} onChange={(event) => setPosition(event.target.value)} className={controlClass}><option value="">ตำแหน่ง</option>{positions.map((item) => <option key={item} value={item}>{item}</option>)}</DropdownSelect></label>
          ...
          <div className="mb-2 grid gap-2 lg:grid-cols-3">
            <label className="flex flex-col text-sm leading-[22px] text-[rgba(0,0,0,0.87)]">บริษัท<DropdownSelect value={selectedCompany} onChange={(event) => setSelectedCompany(event.target.value)} className={controlClass}>{["MIC_ORGANIZE", "MIC", "PSTH_ADMIN", "SVOA", "PECTH"].map((item) => <option key={item}>{item}</option>)}</DropdownSelect></label>
            <label className="flex flex-col text-sm leading-[22px] text-[rgba(0,0,0,0.87)]">โครงสร้างองค์กร<DropdownSelect value={organizationId} onChange={(event) => setOrganizationId(event.target.value)} className={controlClass}><option value="">โครงสร้างองค์กร</option>{organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DropdownSelect></label>
            <label className="flex flex-col text-sm leading-[22px] text-[rgba(0,0,0,0.87)]">ผู้อนุมัติ<DropdownSelect value={newApprover} onChange={(event) => setNewApprover(event.target.value)} className={controlClass}><option value="">ผู้อนุมัติ</option>{approvers.map((approver) => <option key={approver.id} value={approver.id}>{approver.label}</option>)}</DropdownSelect></label>
          </div>
  ...
  return (
    <Card data-leave-quota-calculation className="card-input-container relative mx-4 mb-3 overflow-hidden rounded-lg border-0 bg-white" style={{ boxShadow: "0px 2px 1px -1px rgba(0,0,0,0.2),0px 1px 1px rgba(0,0,0,0.14),0px 1px 3px rgba(0,0,0,0.12)" }}>
      ...
      <CardContent className="card-input-body px-2 py-4 text-sm leading-[22.001px] tracking-[-0.1px] text-[rgba(0,0,0,0.87)]">
        ...
        `}</style>
        <div className="m-6">
          <div className="mb-2 flex flex-col gap-2 sm:flex-row sm:items-end"><label className="flex min-w-0 flex-1 flex-col">โครงสร้างองค์กร<DropdownSelect value={organizationId} onChange={(event) => setOrganizationId(event.target.value)} className={controlClass}><option value="">โครงสร้างองค์กร</option>{organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DropdownSelect></label><label className="flex min-w-0 flex-1 flex-col">Hashtag<input value={hashtag} onChange={(event) => setHashtag(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") setFilters({ organizationId, hashtag }); }} placeholder="#Hashtag" className={controlClass} /></label><button type="button" onClick={() => setFilters({ organizationId, hashtag })} className="h-9 rounded-[4px] bg-[#2299ff] px-4 text-sm font-semibold leading-9 text-white shadow-[0_3px_1px_-2px_rgba(0,0,0,0.2),0_2px_2px_rgba(0,0,0,0.14),0_1px_5px_rgba(0,0,0,0.12)]">ค้นหา</button></div>
      ...
      <CardContent className="card-input-body px-2 py-4 text-sm font-normal leading-[22.001px] tracking-[-0.1px] text-[rgba(0,0,0,0.87)]">
        ...
        <div className="flex flex-col lg:flex-row">
          <section className="m-6 flex min-w-0 flex-1 flex-col">
            <span className="import-no m-2 flex size-10 shrink-0 items-center justify-center rounded-full bg-[#61a8ff] text-xl font-normal text-white">1</span>
            <div className="m-2 min-w-0 flex-1"><h2 className="h-10 text-lg font-bold leading-10">ดาวน์โหลดเทมเพลต (*.xlsx)</h2><div className="flex w-full flex-col gap-2 sm:flex-row sm:items-end"><label className="min-w-0 sm:w-[40%]">โครงสร้างองค์กร<DropdownSelect value={templateOrganizationId} onChange={(event) => setTemplateOrganizationId(event.target.value)} className={controlClass}><option value="">โครงสร้างองค์กร</option>{organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DropdownSelect></label><label className="min-w-0 flex-1">&nbsp;<span className="sr-only">เลือกปี</span><input type="number" min="1900" max="9999" value={year} onChange={(event) => setYear(event.target.value)} placeholder="เลือกปี" className={controlClass} aria-label="เลือกปี" /></label><button type="button" className="h-9 shrink-0 rounded-[4px] bg-[#03ae03] px-4 text-sm font-semibold leading-9 text-white shadow-[0_3px_1px_-2px_rgba(0,0,0,0.2),0_2px_2px_rgba(0,0,0,0.14),0_1px_5px_rgba(0,0,0,0.12)]">ดาวน์โหลดเทมเพลต</button></div></div>
          </section>
  ...
  return (
    ...
    >
      ...
      <CardContent className="card-input-body px-2 py-4 text-sm font-normal leading-[22.001px] tracking-[-0.1px] text-[rgba(0,0,0,0.87)]">
        ...
        `}</style>
        <div className="flex flex-col lg:flex-row">
          <section className="m-6 flex min-w-0 flex-1 flex-col"><span className="import-no m-2 flex size-10 shrink-0 items-center justify-center rounded-full bg-[#61a8ff] text-xl font-normal text-white">1</span><div className="m-2 min-w-0 flex-1"><h2 className="h-10 text-lg font-bold leading-10">ดาวน์โหลดเทมเพลต (*.xlsx)</h2><label>โครงสร้างองค์กร</label><div className="my-2 flex w-full flex-col gap-2 sm:flex-row sm:items-center"><DropdownSelect value={templateOrganizationId} onChange={(event) => setTemplateOrganizationId(event.target.value)} className={`${controlClass} min-w-0 flex-1`}><option value="">โครงสร้างองค์กร</option>{organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DropdownSelect><button type="button" className="h-9 shrink-0 rounded-[4px] bg-[#03ae03] px-4 text-sm font-semibold leading-9 text-white shadow-[0_3px_1px_-2px_rgba(0,0,0,.2),0_2px_2px_rgba(0,0,0,.14),0_1px_5px_rgba(0,0,0,.12)]">เทมเพลตมีรายชื่อพนักงาน</button></div></div></section>
  ...
  return (
    ...
    >
      ...
      <CardContent className="card-input-body px-2 py-4 text-sm font-normal leading-[22.001px] tracking-[-0.1px] text-[rgba(0,0,0,0.87)]">
        ...
        `}</style>
        <div className="flex flex-col lg:flex-row">
          <section className="m-6 flex min-w-0 flex-1 flex-col"><span className="import-no m-2 flex size-10 shrink-0 items-center justify-center rounded-full bg-[#61a8ff] text-xl font-normal text-white">1</span><div className="m-2 min-w-0 flex-1"><h2 className="h-10 text-lg font-bold leading-10">ดาวน์โหลดเทมเพลต (*.xlsx)</h2><div className="my-2 flex w-full flex-col gap-2 sm:flex-row sm:items-end"><label className="min-w-0 flex-1">ประเภทข้อมูล*<DropdownSelect value={downloadDataType} onChange={(event) => setDownloadDataType(event.target.value)} className={controlClass}>{dataTypes.map((type) => <option key={type}>{type}</option>)}</DropdownSelect></label><label className="min-w-0 flex-1">โครงสร้างองค์กร<DropdownSelect value={templateOrganizationId} onChange={(event) => setTemplateOrganizationId(event.target.value)} className={controlClass}><option value="">โครงสร้างองค์กร</option>{organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DropdownSelect></label><button type="button" className="h-9 shrink-0 rounded-[4px] bg-[#03ae03] px-4 text-sm font-semibold leading-9 text-white shadow-[0_3px_1px_-2px_rgba(0,0,0,.2),0_2px_2px_rgba(0,0,0,.14),0_1px_5px_rgba(0,0,0,.12)]">ดาวน์โหลด</button></div></div></section>
      ...
      <CardContent className="card-input-body p-0 text-sm font-normal leading-[22.001px] tracking-[-0.1px] text-[rgba(0,0,0,0.87)]">
        ...
        <div className="flex flex-col lg:flex-row">
          <section className="m-6 flex min-w-0 flex-1 flex-col">
            ...
            <div className="m-2 min-w-0 flex-1"><h2 className="h-10 text-lg font-bold leading-10">ดาวน์โหลดเทมเพลต (*.xlsx)</h2><div className="flex items-end gap-2"><label className="min-w-0 flex-1">โครงสร้างองค์กร<DropdownSelect value={templateOrganizationId} onChange={(event) => setTemplateOrganizationId(event.target.value)} className={controlClass}><option value="">โครงสร้างองค์กร</option>{organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DropdownSelect></label><button type="button" className="h-9 shrink-0 rounded-[4px] bg-[#03ae03] px-4 text-sm font-semibold leading-9 text-white shadow-[0_3px_1px_-2px_rgba(0,0,0,0.2),0_2px_2px_rgba(0,0,0,0.14),0_1px_5px_rgba(0,0,0,0.12)]">ดาวน์โหลด</button></div></div>
          </section>
        ...
        <section className="cost-distribution-workspace">
          <div className="cost-distribution-filters">
            <label className="min-w-0">โครงสร้างองค์กร<DropdownSelect value={organizationId} onChange={(event) => { setOrganizationId(event.target.value); setEmployeeId(""); }} className={controlClass}><option value="">โครงสร้างองค์กรทั้งหมด</option>{organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DropdownSelect></label>
            <label className="min-w-0">ตำแหน่ง<DropdownSelect value={position} onChange={(event) => setPosition(event.target.value)} className={controlClass}><option value="">ตำแหน่งทั้งหมด</option>{positions.map((option) => <option key={option}>{option}</option>)}</DropdownSelect></label>
            <label className="min-w-0">รายชื่อพนักงาน<DropdownSelect value={employeeId} onChange={(event) => setEmployeeId(event.target.value)} className={controlClass}><option value="">ทั้งหมด</option>{availableEmployees.map((employee) => <option key={employee.id} value={employee.id}>{employee.employeeCode}: {employee.name}</option>)}</DropdownSelect></label>
            <label className="min-w-0">Cost Center<DropdownSelect value={costCenter} onChange={(event) => setCostCenter(event.target.value)} className={controlClass}><option value="">ทั้งหมด</option>{costCenterOptions.map((option) => <option key={option}>{option}</option>)}</DropdownSelect></label>
          </div>
      ...
      <CardContent className="card-input-body px-2 py-4">
        <div className="flex flex-col divide-y divide-black/[0.12] lg:flex-row lg:divide-x lg:divide-y-0">
          <section className="m-6 flex min-w-0 flex-1 gap-2 py-2 pr-0 lg:pr-6">
            <span className="import-no m-2 flex size-10 shrink-0 items-center justify-center rounded-full bg-[#61a8ff] text-xl font-normal text-white">1</span>
            <div className="m-2 min-w-0 flex-1"><h2 className="h-10 text-lg font-bold leading-10 text-[rgba(0,0,0,0.87)]">ดาวน์โหลดเทมเพลต (*.xlsx)</h2><div className="mb-2 flex items-end gap-2"><label className="min-w-0 flex-[0_1_40%] text-sm leading-[22px] text-[rgba(0,0,0,0.87)]">โครงสร้างองค์กร<DropdownSelect value={templateOrganizationId} onChange={(event) => setTemplateOrganizationId(event.target.value)} className={controlClass}><option value="">โครงสร้างองค์กร</option>{organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DropdownSelect></label><div className="flex-[0_1_20%]"><label className="block text-sm leading-[22px]">&nbsp;</label><button type="button" className="h-9 max-w-[100px] rounded-[4px] bg-[#03ae03] px-4 text-sm font-semibold leading-9 text-white shadow-[0_3px_1px_-2px_rgba(0,0,0,0.2),0_2px_2px_rgba(0,0,0,0.14),0_1px_5px_rgba(0,0,0,0.12)]">ดาวน์โหลด</button></div></div></div>
          </section>
        ...
        <section className="m-6">
          <div className="mb-2 flex flex-col gap-2 sm:flex-row sm:items-end"><label className="flex min-w-0 flex-1 flex-col text-sm leading-[22px] text-black/87">โครงสร้างองค์กร<DropdownSelect value={organizationId} onChange={(event) => setOrganizationId(event.target.value)} className={controlClass}><option value="">โครงสร้างองค์กร</option>{organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DropdownSelect></label><label className="flex min-w-0 flex-1 flex-col text-sm leading-[22px] text-black/87">Hashtag<input value={hashtag} onChange={(event) => setHashtag(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") setFilters({ organizationId, hashtag }); }} placeholder="#Hashtag" className={controlClass} /></label><button type="button" onClick={() => setFilters({ organizationId, hashtag })} className="h-9 min-w-[64px] rounded-[4px] bg-[#2299ff] px-4 text-sm font-semibold leading-9 text-white shadow-[0_3px_1px_-2px_rgba(0,0,0,0.2),0_2px_2px_rgba(0,0,0,0.14),0_1px_5px_rgba(0,0,0,0.12)] hover:bg-[#1685e8]">ค้นหา</button></div>
  ...
  return (
    <Card data-leave-type-visibility className="card-input-container relative mx-4 mb-3 overflow-hidden rounded-lg border-0 bg-white" style={{ boxShadow: "0px 2px 1px -1px rgba(0,0,0,0.2),0px 1px 1px rgba(0,0,0,0.14),0px 1px 3px rgba(0,0,0,0.12)" }}>
      ...
      <CardContent className="card-input-body px-2 py-4">
        ...
        `}</style>
        <div className="flex flex-col divide-y divide-black/[0.12] lg:flex-row lg:divide-x lg:divide-y-0">
          <section className="m-6 flex min-w-0 flex-1 gap-2 py-2 pr-0 lg:pr-6"><span className="import-no m-2 flex size-10 shrink-0 items-center justify-center rounded-full bg-[#61a8ff] text-xl text-white">1</span><div className="m-2 min-w-0 flex-1"><h2 className="h-10 text-lg font-bold leading-10">ดาวน์โหลดเทมเพลต (*.xlsx)</h2><div className="mb-2 flex items-end gap-2"><label className="min-w-0 flex-[0_1_40%] text-sm leading-[22px]">โครงสร้างองค์กร<DropdownSelect value={templateOrganizationId} onChange={(event) => setTemplateOrganizationId(event.target.value)} className={controlClass}><option value="">โครงสร้างองค์กร</option>{organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DropdownSelect></label><button type="button" className="h-9 max-w-[100px] rounded-[4px] bg-[#03ae03] px-4 text-sm font-semibold text-white shadow-[0_3px_1px_-2px_rgba(0,0,0,0.2),0_2px_2px_rgba(0,0,0,0.14),0_1px_5px_rgba(0,0,0,0.12)]">ดาวน์โหลด</button></div></div></section>
        ...
        <div className="my-6 border-t border-black/[0.12]" />
        <section className="m-6">
          <div className="mb-2 grid gap-2 md:grid-cols-2 xl:grid-cols-[repeat(4,minmax(0,1fr))_auto] xl:items-end"><label className="text-sm leading-[22px]">ชื่อพนักงาน<input value={filters.name} onChange={(event) => setFilters((current) => ({ ...current, name: event.target.value }))} placeholder="ค้นหาชื่อพนักงาน..." className={controlClass} /></label><label className="text-sm leading-[22px]">โครงสร้างองค์กร<DropdownSelect value={filters.organizationId} onChange={(event) => setFilters((current) => ({ ...current, organizationId: event.target.value }))} className={controlClass}><option value="">โครงสร้างองค์กร</option>{organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DropdownSelect></label><label className="text-sm leading-[22px]">โครงสร้างตำแหน่ง<DropdownSelect value={filters.position} onChange={(event) => setFilters((current) => ({ ...current, position: event.target.value }))} className={controlClass}><option value="">โครงสร้างตำแหน่ง</option>{positionOptions.map((position) => <option key={position}>{position}</option>)}</DropdownSelect></label><label className="text-sm leading-[22px]">Hashtag<input value={filters.hashtag} onChange={(event) => setFilters((current) => ({ ...current, hashtag: event.target.value }))} onKeyDown={(event) => { if (event.key === "Enter") setAppliedFilters(filters); }} placeholder="#Hashtag" className={controlClass} /></label><button type="button" onClick={() => setAppliedFilters(filters)} className="h-9 rounded-[4px] bg-[#2299ff] px-4 text-sm font-semibold text-white shadow-[0_3px_1px_-2px_rgba(0,0,0,0.2),0_2px_2px_rgba(0,0,0,0.14),0_1px_5px_rgba(0,0,0,0.12)]">ค้นหา</button></div>
    ...
    <Card
      ...
      <CardContent className="card-input-body px-2 pt-4 pb-[6px] text-sm font-normal leading-[22.001px] tracking-[-0.1px] text-[rgba(0,0,0,0.87)]">
        <div className="mx-6 mt-6 mb-0">
          <div className="mb-2 flex flex-col gap-2 sm:flex-row sm:items-end">
            <label className="flex min-w-0 flex-1 flex-col text-sm leading-[22px] text-black/87">โครงสร้างองค์กร
              <DropdownSelect value={organizationId} onChange={(event) => setOrganizationId(event.target.value)} className={controlClass}>
                <option value="">โครงสร้างองค์กร</option>
                {organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}
              </DropdownSelect>
            </label>
...
function WelfareSettingsContent({ orgTree }: { orgTree: OrgNode[] }) {
  ...
  const yearSelect = (value: string, onChange: (value: string) => void, placeholder: string, testId?: string) => (
    <DropdownSelect data-testid={testId} value={value} onChange={(event) => { onChange(event.target.value); setSaved(false); }} className={controlClass}>
      ...
      <CardContent className="card-input-body px-2 py-4 text-sm font-normal leading-[22.001px] tracking-[-0.1px] text-[rgba(0,0,0,0.87)]">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="welfare-template-fields mb-2 flex flex-col gap-2">
                <label className="text-sm leading-[22px]">โครงสร้างองค์กร <span className="text-red-600">*</span>
                  <DropdownSelect value={templateOrganizationId} onChange={(event) => { setTemplateOrganizationId(event.target.value); setSaved(false); }} className={controlClass}>
                    <option value="">โครงสร้างองค์กร</option>
                    {organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}
                  </DropdownSelect>
                </label>
                <div className="grid items-end gap-2 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto]">
                  <label className="text-sm leading-[22px]">ประเภทสวัสดิการ <span className="text-red-600">*</span>
                    <DropdownSelect value={templateWelfareType} onChange={(event) => { setTemplateWelfareType(event.target.value); setSaved(false); }} className={controlClass}>
        ...
        <div className="m-6 mb-0">
          <div className="mb-2 grid gap-2 sm:grid-cols-2 xl:grid-cols-[repeat(4,minmax(0,1fr))_auto] xl:items-end">
            ...
            <label className="text-sm leading-[22px]">ประเภทสวัสดิการ <span className="text-red-600">*</span>
              <DropdownSelect data-testid="wcl-bulk-type-select" value={welfareType} onChange={(event) => { setWelfareType(event.target.value); setSaved(false); }} className={controlClass}>
                <option value="">ประเภทสวัสดิการ</option>
                {welfareTypes.map((type) => <option key={type} value={type}>{type}</option>)}
              </DropdownSelect>
            </label>
            ...
            <div className="m-2 min-w-0 flex-1">
              ...
              <div className="flex items-end gap-2">
                <label className="min-w-0 flex-[0_1_70%] text-sm leading-[22px] text-black/87">โครงสร้างองค์กร
                  <DropdownSelect value={templateOrganizationId} onChange={(event) => setTemplateOrganizationId(event.target.value)} className={controlClass}>
                    <option value="">โครงสร้างองค์กร</option>
                    {organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}
                  </DropdownSelect>
                </label>
  ...
  return (
    ...
    >
      ...
      <CardContent className="card-input-body px-2 py-4">
        <div className="m-6">
          <div className="mb-2 flex flex-col gap-2 lg:flex-row lg:items-end">
            <label className="flex-1 text-sm leading-[22.001px] text-black/85">โครงสร้างองค์กร<DropdownSelect value={organizationId} onChange={(event) => setOrganizationId(event.target.value)} className={controlClass}><option value="">โครงสร้างองค์กร</option>{organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DropdownSelect></label>
            ...
            <label className="flex-1 text-sm leading-[22.001px] text-black/85">รอบการคำนวณเงินเดือน<DropdownSelect value={payrollFilter} onChange={(event) => setPayrollFilter(event.target.value)} className={controlClass}><option>ทั้งหมด</option><option>เต็มงวด</option><option>แบ่งงวดจ่าย</option></DropdownSelect></label>
    ...
    >
      ...
      <CardContent className="card-input-body px-2 py-4">
        <div className="m-6 flex flex-col divide-y divide-black/[0.12] lg:flex-row lg:divide-x lg:divide-y-0">
          <section className="flex flex-1 gap-2 py-2 pr-0 lg:pr-6"><span className="import-no m-2 flex size-10 shrink-0 items-center justify-center rounded-full bg-[#61a8ff] text-xl text-white">1</span><div className="m-2 min-w-0 flex-1"><h2 className="h-10 text-lg font-bold leading-10">ดาวน์โหลดเทมเพลต (*.xlsx)</h2><div className="mb-2 flex items-end gap-2"><label className="min-w-0 flex-[0_1_40%] text-sm leading-[22px]">โครงสร้างองค์กร<DropdownSelect value={templateOrg} onChange={(event) => setTemplateOrg(event.target.value)} className={controlClass}><option value="">โครงสร้างองค์กร</option>{organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DropdownSelect></label><button type="button" className="h-9 max-w-[100px] rounded-[4px] bg-[#03ae03] px-4 text-sm font-semibold text-white shadow-[0_3px_1px_-2px_rgba(0,0,0,0.2),0_2px_2px_rgba(0,0,0,0.14),0_1px_5px_rgba(0,0,0,0.12)]">ดาวน์โหลด</button></div></div></section>
        ...
        <div className="m-6">
          <div className="mb-2 flex flex-col gap-2 lg:flex-row lg:items-end"><label className="flex-1 text-sm leading-[22.001px] text-black/85">โครงสร้างองค์กร<DropdownSelect value={organizationId} onChange={(event) => setOrganizationId(event.target.value)} className={controlClass}><option value="">โครงสร้างองค์กร</option>{organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DropdownSelect></label><label className="flex-1 text-sm leading-[22.001px] text-black/85">ประเภทรายการรายรับ-รายจ่ายคงที่<DropdownSelect value={itemType} onChange={(event) => { setItemType(event.target.value); setSaved(false); }} className={controlClass}><option value="" /><option value="ค่าเดินทาง">ค่าเดินทาง</option><option value="ค่าโทรศัพท์">ค่าโทรศัพท์</option><option value="เบี้ยเลี้ยง">เบี้ยเลี้ยง</option></DropdownSelect></label><label className="flex-1 text-sm leading-[22.001px] text-black/85">Hashtag<input value={hashtag} onChange={(event) => setHashtag(event.target.value)} placeholder="#Hashtag" className={controlClass} /></label><button type="button" onClick={() => void search()} className="h-9 shrink-0 rounded-[4px] bg-[#2299ff] px-4 text-sm font-semibold text-white shadow-[0_3px_1px_-2px_rgba(0,0,0,0.2),0_2px_2px_rgba(0,0,0,0.14),0_1px_5px_rgba(0,0,0,0.12)]">ค้นหา</button></div>
...
function WorkShiftVisibilityContent({ orgTree, companyId }: { orgTree: OrgNode[]; companyId: string }) {
  ...
  return (
    <Card className="card-input-container relative mx-4 mb-3 overflow-hidden rounded-lg border-0 bg-white" style={{ boxShadow: "0px 2px 1px -1px rgba(0,0,0,0.2),0px 1px 1px rgba(0,0,0,0.14),0px 1px 3px rgba(0,0,0,0.12)" }}>
      ...
      <CardContent className="card-input-body px-2 py-4">
        <div className="flex flex-col divide-y divide-black/[0.12] lg:flex-row lg:divide-x lg:divide-y-0">
          <section className="m-6 flex flex-1 gap-2 py-2 pr-0 lg:pr-6"><span className="import-no m-2 flex size-10 shrink-0 items-center justify-center rounded-full bg-[#61a8ff] text-xl font-normal text-white">1</span><div className="m-2 min-w-0 flex-1"><h2 className="h-10 text-lg font-bold leading-10 text-[rgba(0,0,0,0.87)]">ดาวน์โหลดเทมเพลต (*.xlsx)</h2><div className="mb-2 flex items-end gap-2"><label className="min-w-0 flex-[0_1_40%] text-sm leading-[22px] text-[rgba(0,0,0,0.87)]">โครงสร้างองค์กร<DropdownSelect value={templateOrganizationId} onChange={(event) => setTemplateOrganizationId(event.target.value)} className={controlClass}><option value="">โครงสร้างองค์กร</option>{organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DropdownSelect></label><div className="flex-[0_1_20%]"><label className="block text-sm leading-[22px]">&nbsp;</label><button type="button" className="h-9 max-w-[100px] rounded-[4px] bg-[#03ae03] px-4 text-sm font-semibold leading-9 text-white shadow-[0_3px_1px_-2px_rgba(0,0,0,0.2),0_2px_2px_rgba(0,0,0,0.14),0_1px_5px_rgba(0,0,0,0.12)]">ดาวน์โหลด</button></div></div></div></section>
        ...
        <div className="my-6 border-t border-black/[0.12]" />
        <div className="m-6"><div className="mb-2 flex flex-col gap-2 sm:flex-row sm:items-end"><label className="flex min-w-0 flex-1 flex-col text-sm leading-[22px] text-black/87">โครงสร้างองค์กร<DropdownSelect value={organizationId} onChange={(event) => setOrganizationId(event.target.value)} className={controlClass}><option value="">โครงสร้างองค์กร</option>{organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DropdownSelect></label><label className="flex min-w-0 flex-1 flex-col text-sm leading-[22px] text-black/87">Hashtag<input value={hashtag} onChange={(event) => setHashtag(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") setFilters({ organizationId, hashtag }); }} placeholder="#Hashtag" className={controlClass} /></label><button type="button" onClick={() => setFilters({ organizationId, hashtag })} className="h-9 min-w-[64px] rounded-[4px] bg-[#2299ff] px-4 text-sm font-semibold leading-9 text-white shadow-[0_3px_1px_-2px_rgba(0,0,0,0.2),0_2px_2px_rgba(0,0,0,0.14),0_1px_5px_rgba(0,0,0,0.12)] hover:bg-[#1685e8]">ค้นหา</button></div>
        ...
        <div className="m-6">
          <div className="flex flex-col divide-y divide-black/[0.12] lg:flex-row lg:divide-x lg:divide-y-0">
            <section className="flex flex-1 gap-2 py-2 pr-0 lg:pr-6">
              <span className="import-no m-2 flex size-10 shrink-0 items-center justify-center rounded-full bg-[#61a8ff] text-xl font-normal text-white">1</span>
              <div className="m-2 min-w-0 flex-1"><h2 className="h-10 text-lg font-bold leading-10 text-[rgba(0,0,0,0.87)]">ดาวน์โหลดเทมเพลต (*.xlsx)</h2><div className="flex items-end gap-2"><label className="min-w-0 flex-[0_1_70%] text-sm leading-[22px] text-[rgba(0,0,0,0.87)]">โครงสร้างองค์กร<DropdownSelect value={templateOrganizationId} onChange={(event) => setTemplateOrganizationId(event.target.value)} className={controlClass}><option value="">โครงสร้างองค์กร</option>{organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DropdownSelect></label><button type="button" className="h-9 rounded-[4px] bg-[#03ae03] px-4 text-sm font-semibold leading-9 text-white shadow-[0_3px_1px_-2px_rgba(0,0,0,0.2),0_2px_2px_rgba(0,0,0,0.14),0_1px_5px_rgba(0,0,0,0.12)]">ดาวน์โหลด</button></div></div>
            </section>
          ...
          <div className="my-6 border-t border-black/[0.12]" />
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end"><label className="flex min-w-0 flex-1 flex-col text-sm leading-[22px] text-black/87">โครงสร้างองค์กร<DropdownSelect value={organizationId} onChange={(event) => setOrganizationId(event.target.value)} className={controlClass}><option value="">โครงสร้างองค์กร</option>{organizationOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</DropdownSelect></label><label className="flex min-w-0 flex-1 flex-col text-sm leading-[22px] text-black/87">Hashtag<input value={hashtag} onChange={(event) => setHashtag(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") setFilters({ organizationId, hashtag }); }} placeholder="#Hashtag" className={controlClass} /></label><button type="button" onClick={() => setFilters({ organizationId, hashtag })} className="h-9 min-w-[64px] rounded-[4px] bg-[#2299ff] px-4 text-sm font-semibold leading-9 text-white shadow-[0_3px_1px_-2px_rgba(0,0,0,0.2),0_2px_2px_rgba(0,0,0,0.14),0_1px_5px_rgba(0,0,0,0.12)] hover:bg-[#1685e8]">ค้นหา</button></div>
          <label className="mt-2 flex flex-col text-sm leading-[22px] text-black/87">กะการทำงาน <span className="text-red-600">***เพิ่มกะการทำงานได้ที่นี่ <a href="/organization/organization-workcycle" className="text-[#2299ff] underline">Link</a></span><DropdownSelect value={selectedShift} onChange={(event) => setSelectedShift(event.target.value)} className={controlClass}><option>WC001</option><option>WC002</option></DropdownSelect></label>
