...
const TERMINATION_REASONS = ["ทั้งหมด", "ลาออก", "เกษียณอายุ", "เลิกจ้าง"];
const EMPLOYEE_REASONS = ["ทั้งหมด", "เหตุผลส่วนตัว", "ย้ายงาน", "ไม่ผ่านทดลองงาน"];
...
export default function ReportEmployeeTerminationsPage() {
    ...
    <div>
      <section className="relative h-40 border-b border-border bg-[#51A0F5] p-6 text-white">
        <div className="flex h-full flex-col items-start justify-center">
          ...
          <h1 className="mt-4 text-2xl font-bold text-white">รายงานพนักงานลาออก</h1>
        </div>
      </section>
        ...
        <Card className="min-w-[1163px] overflow-hidden">
          ...
          <CardContent className="px-4 pb-4 pt-4">
            ...
            <div className="mt-[27px] flex justify-end gap-2">
              <Button disabled className="h-9 w-[86px] rounded-[3px] bg-[#485166] px-3.5 text-sm shadow-sm disabled:bg-[#485166] disabled:text-white disabled:opacity-100">PDF<FileText className="size-4" /></Button>
              <Button disabled className="h-9 w-[90px] rounded-[3px] bg-[#43b14b] px-3.5 text-sm shadow-sm disabled:bg-[#a5d6a7] disabled:text-white disabled:opacity-100">Excel<Grid3x3 className="size-4" /></Button>
              <Button className="h-9 w-[65px] rounded-[3px] bg-[#1890ff] px-3.5 text-sm shadow-sm hover:bg-[#1890ff]/90">ค้นหา</Button>
            </div>
          </CardContent>
        </Card>
        <Card className="min-w-[1163px]"><CardContent className="px-2 pb-2 pt-0"><EmptyState /></CardContent></Card>
      </div>
    </div>
  );
}
