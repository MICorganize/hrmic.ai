...
import loginIllustration from "@/public/images/auth/login-illustration.jpg";
...
export function LoginForm({ initialCompanies, kind }: LoginFormProps) {
  ...
  const copy = language === "en"
    ? isCompanyManagement
      ...
      : {
          language: "Language", imageAlt: "Illustration of secure sign in", title: "Log in", subtitle: "", company: "Select company", username: "Username", usernamePlaceholder: "Email or username", password: "Password", passwordPlaceholder: "Enter password", showPassword: "Show password", hidePassword: "Hide password", remember: "Remember me", forgotPassword: "Forgot password?", submit: "Log in", loading: "Signing in", noAccount: "Don't have an account?", alternateLogin: "Company Management", invalidCredentials: "Incorrect email or password", companyAccessDenied: "Your account cannot access the selected company.", genericError: "Something went wrong. Please try again.",
        }
    : isCompanyManagement
      ? {
  ...
  return (
    <main lang={language} className="grid h-dvh overflow-hidden place-items-center bg-[#f4f5f7] p-4 font-[family:var(--font-kanit)] text-[#454545] sm:p-8">
      <section className="grid w-full max-w-[960px] overflow-hidden rounded-xl bg-white shadow-[0_4px_24px_rgba(33,41,52,0.14)] lg:grid-cols-2">
        <aside className="hidden min-h-full overflow-hidden bg-[#012458] lg:flex lg:items-center">
          <Image src={loginIllustration} alt={copy.imageAlt} loading="eager" sizes="(min-width: 1024px) 480px, 1px" className="h-auto w-full" />
        </aside>

        <div className={cn("relative flex flex-col justify-center px-7 py-6 sm:px-12", isCompanyManagement && "lg:justify-start")}>
          ...
          <div className="mb-4 sm:mb-5">
            <div className={cn(isCompanyManagement ? "origin-center -translate-y-4 scale-[.64] sm:scale-[.72]" : "flex h-16 items-center justify-center")}>
              <div className={cn(!isCompanyManagement && "origin-center scale-[.72] sm:scale-[.76]")}>
                <HRMicWordmark primaryClassName="text-[#315ff4] [text-shadow:0_0_1.5px_#1849db]" taglineClassName={cn(!isCompanyManagement && "mt-3 leading-none")} />
              </div>
            </div>
            <h1 className={cn("text-2xl font-semibold text-[#333]", isCompanyManagement ? "-mt-4" : "mt-1")}>{copy.title}</h1>
            {copy.subtitle && <p className="mt-1 text-sm text-[#777]">{copy.subtitle}</p>}
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                <button type="button" disabled={companiesLoading || companies.length === 0} onClick={() => setCompanyOpen((open) => !open)} aria-label={copy.company} className="flex h-10 w-full items-center justify-between rounded-xl border border-[#e0e3e7] px-4 text-sm text-[#555d66] transition-colors hover:border-[#a8b3c3] disabled:cursor-wait disabled:opacity-60">
                  <span className={cn("flex items-center gap-3 text-sm font-normal", !company && "text-[#9ca3af]")}><List className="size-4 text-[#5d6670]" strokeWidth={1.8} />{company?.code ?? copy.company}</span>
                ...
                {companyOpen && <div className="absolute z-20 mt-1 w-full overflow-hidden rounded-xl border border-[#dce2e6] bg-white py-1 shadow-lg">
                  {companies.map((option) => <button key={option.id} type="button" onClick={() => { setCompany(option); setCompanyOpen(false); }} className="flex w-full items-center justify-between px-4 py-2 text-left text-sm font-normal text-[#4c535b] hover:bg-[#edf4ff]"><span className="truncate">{option.code}</span>{option.id === company?.id && <Check className="size-4 shrink-0 text-[#135ee4]" />}</button>)}
                </div>}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input id={emailFieldId} name="email" type="email" autoComplete="username" placeholder={copy.usernamePlaceholder} value={email} onChange={(event) => setEmail(event.target.value)} className="h-10 w-full rounded-xl border border-[#e0e3e7] bg-white pl-11 pr-4 text-sm font-normal text-[#353a40] outline-none transition placeholder:text-sm placeholder:font-normal placeholder:text-[#9ca3af] focus:border-[#135ee4] focus:ring-1 focus:ring-[#135ee4]" required />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input id={passwordFieldId} name="password" type={showPassword ? "text" : "password"} autoComplete="current-password" placeholder={copy.passwordPlaceholder} value={password} onChange={(event) => setPassword(event.target.value)} className="h-10 w-full rounded-xl border border-[#e0e3e7] bg-white pl-11 pr-11 text-sm text-[#353a40] outline-none transition focus:border-[#135ee4] focus:ring-1 focus:ring-[#135ee4]" required />
            ...
            <button type="submit" disabled={loading || (!isCompanyManagement && !company)} aria-disabled={!email || !password || (!isCompanyManagement && !company)} className={cn("flex h-10 w-full items-center justify-center gap-2 rounded-xl text-[16px] font-medium text-white shadow-sm transition", email && password && (isCompanyManagement || company) ? "bg-[#315ff4] hover:bg-[#1849db]" : "bg-[#b8c5ed]")}>{loading && <Loader2 className="size-4 animate-spin" />}{loading ? copy.loading : copy.submit}</button>
          </form>

          {isCompanyManagement && <div className="my-6 flex items-center gap-3 text-[11px] uppercase tracking-wide text-[#b1b5ba]"><span className="h-px flex-1 bg-[#e4e6e8]" />HRMic.ai<span className="h-px flex-1 bg-[#e4e6e8]" /></div>}
          <p className={cn("text-center text-sm text-[#757b82]", isCompanyManagement ? "-mt-[15px]" : "mt-5")}>
            {copy.noAccount}{" "}
          ...
          <Link href={alternateLoginHref} draggable={false} className="mt-4 flex h-10 w-full items-center justify-center rounded-xl bg-[#fc9a12] text-[14px] font-medium text-white shadow-sm transition hover:bg-[#e88a0a]">
            {copy.alternateLogin}
          </Link>
        </div>
      </section>
